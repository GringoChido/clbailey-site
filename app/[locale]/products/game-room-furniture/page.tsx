...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description:
      "Spectator chairs, storage benches, and matching cue racks in solid hardwood. Designed to complement your C.L. Bailey pool table. Dealer exclusive.",
    openGraph: {
      title: "Game Room Furniture | C.L. Bailey & Co.",
      description:
        "Spectator chairs, storage benches, and matching cue racks in solid hardwood. Designed to complement your C.L. Bailey pool table.",
      type: "website",
    },
    alternates: {
      canonical: `https://clbailey.com/${locale}/products/game-room-furniture`,
      languages: {
        en: "https://clbailey.com/en/products/game-room-furniture",
        es: "https://clbailey.com/es/products/game-room-furniture",
      },
    },
  };
}

export default async function GameRoomFurniturePage({ params }: PageProps) {
  const { locale } = await params;
      ...
      <section className="relative w-full h-[50vh] min-h-[360px] flex items-end overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-[90rem] mx-auto px-6 lg:px-10 pb-14">
          ...
          <h1 className="heading-display text-3xl md:text-[2.75rem] lg:text-5xl text-white mb-3">
            {tc("gameRoomFurnitureName")}
          </h1>
          <p className="text-base text-white/50 max-w-lg">
            {tc("gameRoomFurnitureDesc")}
          </p>
        </div>
      </section>
